...
//与触摸屏芯片连接引脚	   
//与触摸屏芯片连接引脚	   
#define PEN (PEN_GPIO_Port->IDR & PEN_Pin ? 1 : 0)
#define DOUT       (DOUT_GPIO_Port->IDR & DOUT_Pin ? 1 : 0)
#define TDIN(x)    TDIN_GPIO_Port -> BSRR = TDIN_Pin << (16 * (!x))
#define TCLK(x)    TCLK_GPIO_Port -> BSRR = TCLK_Pin << (16 * (!x)) 
#define TCS(x)     TCS_GPIO_Port ->  BSRR = TCS_Pin  << (16 * (!x)) 
